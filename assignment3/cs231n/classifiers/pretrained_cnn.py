...
class PretrainedCNN(object):
  ...
  def guided_backprop(self, dout, cache):
    """
    Run the model backward over a sequence of layers that were previously run
    forward using the self.forward method.
    ...
    for i in reversed(range(start, end + 1)):
      ...
      if i == len(self.conv_params) + 1:
        ...
        dcond = (dnext_a>0)*1
        da_bn = relu_backward(dnext_a, relu_cache) * dcond
        da, dgamma, dbeta = batchnorm_backward(da_bn, bn_cache)
        dprev_a, dw, db = affine_backward(da, fc_cache)
        ...
        dcond = (dnext_a>0)*1
        dan = relu_backward(dnext_a, relu_cache) * dcond
        da, dgamma, dbeta = spatial_batchnorm_backward(dan, bn_cache)
        dprev_a, dw, db = conv_backward_fast(da, conv_cache)
